...
if (isset($_POST['pay']))
 {
    extract($_POST);
    $po="select * from room  INNER JOIN `subcat`  USING (`subcat_id`) INNER JOIN `category`  USING (`cat_id`)  where room_id='$rid'";
    $pi=select($po);
    $m=$pi[0]['npa'];
    $n=$pi[0]['cat_type'];
    if($n==$m)
...
{
    extract($_GET);
    $a="update room set room_status='available',npa=npa-'1' where room_id='$cid'";
    update($a);
    $e="delete from booking where room_id='$cid' and student_id='$stuid'";
    delete($e);
    alert("Room Canceled  , refund with in 24 hrs.......");
    return redirect("student_home.php");
}
...
<div class="row " >
                ...
                <div class="description col-md-8" style="float: right;" >
                    ...
                    }else if($row['bstatus']=='paid') 
                    {
                       ...
                       <div><td><a href="?cid=<?php echo $row['room_id']?>&stid=<?php echo $row['student_id']?>" class="btn btn-success"> Cancel Room</a></td></div>
                    <?php
                    }
                    ?>
